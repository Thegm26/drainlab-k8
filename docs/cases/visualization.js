...
const labels = {
  ...
  topology: ['Case 4 · Topology spread', 'Four replicas, balanced across two workers', 'A maxSkew: 1 spread rule allows more replicas while keeping the workers balanced.']
}[type];
...
function render() {
  const protectedMode = type === 'required' || type === 'ignored';
  const topologyMode = type === 'topology';
  const pods1 = topologyMode ? Array.from({ length: Math.ceil(state.replicas / 2) }, (_, i) => pod(`web-${i * 2 + 1}`, state.failed1 ? 'Lost' : 'Running')).join('') : protectedMode ? pod('web-1', state.failed1 ? 'Lost' : 'Running') : Array.from({ length: state.replicas }, (_, i) => pod(`web-${i + 1}`, state.failed1 ? 'Lost' : 'Running')).join('');
  const pods2 = topologyMode ? Array.from({ length: Math.floor(state.replicas / 2) }, (_, i) => pod(`web-${i * 2 + 2}`, state.failed2 ? 'Lost' : 'Running')).join('') : protectedMode ? pod('web-2', state.failed2 ? 'Lost' : 'Running') : '';
  const unavailable = Number(state.failed1) + Number(state.failed2);
  const pending = protectedMode ? Math.max(0, state.replicas - 2) + unavailable : topologyMode ? (state.failed1 ? Math.ceil(state.replicas / 2) : 0) + (state.failed2 ? Math.floor(state.replicas / 2) : 0) : 0;
  const available = protectedMode ? 2 - unavailable : topologyMode ? (state.failed1 ? 0 : Math.ceil(state.replicas / 2)) + (state.failed2 ? 0 : Math.floor(state.replicas / 2)) : state.failed1 ? 0 : state.replicas;
  ...
  const message = type === 'ignored' && state.changed ? `<strong>web service: ${available} / ${state.replicas} Running</strong> · no Pods evicted` : pending ? `<strong>web service: ${available} / ${state.replicas} Running</strong> · ${pending} Pending` : topologyMode ? `<strong>web service: ${available} / ${state.replicas} Running</strong> · balanced across workers` : `<strong>web service: ${available} / ${state.replicas} Running</strong>`;
  const previous = { without: '../visualization.html', required: 'without-anti-affinity.html', ignored: 'required-pod-anti-affinity.html', topology: 'ignored-during-execution.html' }[type];
  const next = { without: 'required-pod-anti-affinity.html', required: 'ignored-during-execution.html', ignored: 'topology-spread.html', topology: '../visualization.html' }[type];
  const nextLabel = type === 'topology' ? 'Main menu ↑' : 'Next case →';
  app.innerHTML = `<header class="page-head"><div><p class="eyebrow">${labels[0]}</p><h1>${labels[1]}</h1><p class="case-copy">${labels[2]}</p></div><a class="menu-card" href="../visualization.html"><span>Main menu</span><strong>Choose a case</strong><small>Lab 01 scenarios</small></a></header><div class="controls"><button data-action="scale">Scale +1</button><button class="fail" data-action="fail1">Fail w1</button><button class="fail" data-action="fail2">Fail w2</button>${type === 'ignored' ? '<button data-action="change">Change label</button>' : ''}<button data-action="reset">Reset</button></div><section class="cluster"><div class="cluster-head"><strong>web service</strong><span>${available} / ${state.replicas} Running</span></div><div class="nodes">${node('worker-1', state.failed1, pods1, '')}${node('worker-2', state.failed2, pods2, type === 'ignored' && state.changed ? label2 : '', type === 'ignored' && state.changed)}</div>${queue}</section><p class="outcome ${available === 0 ? 'down' : type === 'ignored' && state.changed ? 'note' : ''}">${message}</p><nav class="pager"><a href="${previous}">← Previous</a><a href="${next}">${nextLabel}</a></nav>`;
